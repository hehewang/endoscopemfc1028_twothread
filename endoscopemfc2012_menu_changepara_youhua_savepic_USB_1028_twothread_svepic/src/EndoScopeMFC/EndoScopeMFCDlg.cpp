...
UINT CEndoScopeMFCDlg::SavePicThreadProc(LPVOID lpParameter)
{
	...
	Mat img = Mat(IMGHEIGHT, IMGWIDTH, CV_8UC3,IMGSIZE);
	string t_imgpath = pdlg->GetSaveImgPath(1);

	while (pdlg->m_bstartpicture)
	{
		BYTE* t_pbuf=pdlg->GetPicture();
		if (t_pbuf == nullptr)
		{
			continue;
		}
		memcpy(img.data, t_pbuf, IMGSIZE);
		if (!img.empty())
		{
...
void CEndoScopeMFCDlg::ShotPicWindow()
{
	...
	if (SUCCEEDED(hr))
	{
		string t_imgpath = GetSaveImgPath();
		SaveCaptureImage(t_imgpath);
	}
	return hr;*/
	/*回调方式*/
	...
	if (!bshot)
	{
		...
		//string t_imgpath = GetSaveImgPath(1);
		//if(!t_imgpath.empty())
		//{
...
UINT CEndoScopeMFCDlg::ShowVideoThreadFunc(LPVOID lpParameter)
{
	...
	while(pObj->m_bshowvideo)
	{
		...
		if(pObj->m_bwritepicture)
		{
			unsigned char* pbuf = new unsigned char[IMGSIZE];
			memset(pbuf, 0, IMGSIZE);
			memcpy(pbuf, m_showrc.data, IMGSIZE);
			//	WaitForSingleObject(g_picevent, INFINITE);
			pObj->m_pPicList.push_back(pbuf);//操作队列
			SetEvent(g_picevent);
			pObj->m_bwritepicture = false;
...
void CEndoScopeMFCDlg::RestoreFactorySetting()//出厂设置
{
	...
	Sleep(20);
	//13.色调R B
	int nRed = GetPrivateProfileInt(_T("PARAMETER"), _T("HUE_R"), 1, g_ConfigPath);
	if((nRed<-9)||(nRed>9))
	{
		nRed = 0;
	}
	...
	int nBlue = GetPrivateProfileInt(_T("PARAMETER"), _T("HUE_B"), 1, g_ConfigPath);
	if((nBlue<-9)||(nBlue>9))
	{
		nBlue = 0;
	}
	...
	//9.伽马设置
	//10.测光模式
	int nMetering = GetPrivateProfileInt(_T("PARAMETER"), _T("ADJBRIGHT"), 1, g_ConfigPath);
	if(0 == nMetering)
	{
		g_nMetering = 0;
		g_pPLC->AverageMetering();
	...
	Sleep(20);
	//11.轮廓
	int nShowShape = GetPrivateProfileInt(_T("PARAMETER"), _T("CONTUOR"), 0, g_ConfigPath);
	if(0 == nShowShape)
	{
		m_nPicWindowShape = 1;//设置成圆角

	}
	else if(1 == nShowShape)
	{
		m_nPicWindowShape = 0;//设置成方角
	}
	SetPicWidowShape();
	Sleep(20);
	//12.放大
...
//	static BOOL bStart=FALSE;
	...
	if (!bSaveVideoStart)//启动线程
	{
		...
		string t_imgpath = GetSaveImgPath(2);
